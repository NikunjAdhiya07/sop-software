...
// GET /api/training-matrix/matrix-entries?department=QA&month=5&year=2025&employee=John&sopCode=QAGE01
...
// POST /api/training-matrix/matrix-entries — create a single matrix entry
export async function POST(req: NextRequest) {
  try {
    ...
    const entry = await MatrixEntryData.create({
      department,
      employeeName,
...
// PUT /api/training-matrix/matrix-entries — upsert or bulk-update entries
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (Array.isArray(body.entries)) {
      ...
      const result = await MatrixEntryData.bulkWrite(ops);
      return NextResponse.json({
...
// DELETE /api/training-matrix/matrix-entries?id=xxx&deletedBy=admin  (soft delete)
